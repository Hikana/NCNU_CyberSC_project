...
<div class="flex-1 flex items-center justify-center px-8 py-16">
  <div
    class="max-w-4xl w-full bg-white/10 backdrop-blur-md rounded-2xl shadow-lg p-8"
  >
    ...
    <div v-if="alicePublic" class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
      <div>
        <label class="block font-semibold mb-2">Alice Public Key</label>
        <textarea
          readonly
      ...
      <div>
        <label class="block font-semibold mb-2">Bob Public Key</label>
        <textarea
          readonly
          class="w-full h-32 p-2 rounded bg-black/70 text-green-300 font-mono text-xs"
        >{{ bobPublic }}</textarea>
