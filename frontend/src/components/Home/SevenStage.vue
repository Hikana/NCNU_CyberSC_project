<template>
  ...
  >
    ...
    <div class="absolute left-16 top-[10%] flex items-start z-20 scale-125">
      <div
    ...
    >
      <div class="relative bg-lightGray rounded-xl p-8 flex flex-col w-[72vw] max-w-[1100px] max-h-[75vh] overflow-hidden">
        <h2
          class="text-3xl font-bold text-wordcolor text-center mb-4 leading-snug"
          v-html="selectedStep.base"
        ></h2>
...
const steps = [
  ...
  {base:"SSL (Secure Sockets Layer) - 安全通訊協定", detail:"這是一種<strong>已被淘汰且不安全</strong>的早期加密協定。 它曾被用於網站加密（HTTPS），但因被發現有嚴重漏洞，現已停止使用。", color:colorGroups.group6, layer: '表示層'},
  {base:"TLS (Transport Layer Security) - 傳輸層安全性協定", detail:"這是 <strong>SSL 的現代安全後繼者</strong>。 它是<strong>當前網站 HTTPS 加密的標準技術</strong>，也就是您在瀏覽器上看到的那個「鎖頭」，用於保護您的 transmission 資料安全。", color:colorGroups.group6, layer: '表示層'},
  ...
  {
  base: "Web Session（網站會話）",
  detail: "當你登入一個網站後，它會建立一個「會話（Session）」用來記住你是誰。這層負責<strong>維持你與伺服器之間的對話狀態</strong>，例如登入狀態、購物車內容等。常透過 Cookie、Token 或 Session ID 來管理。",
  color: colorGroups.group5,
  layer: "會話層"},
  ...
  {base:"UDP (User Datagram Protocol) - 使用者資料包協定", detail:"這像是「平信」或「明信片」。 它是一種<strong>快速、無連線</strong>的協定，抓到資料就直接往外丟，<strong>不保證資料完整送達</strong>與順序。 它追求的是<strong>低延遲和高效率</strong>，適用於線上遊戲、直播等即時性應用（寧可犧牲一點畫質，也不能卡頓）。", color:colorGroups.group4, layer: '傳輸層'},
  ...
  {base:"IPv4 (Internet Protocol version 4)", detail:"<strong>舊版</strong>的IP位址系統，使用 <strong>32 位元</strong>位址（由四組 0-255 的數字組成）。例如：<code>192.168.1.1</code>。 它大約只能提供 43 億個位址，目前已全球耗盡。", color:colorGroups.group3, layer: '網路層'},
  {base:"IPv6 (Internet Protocol version 6)", detail:"<strong>新版</strong>的IP位址系統，使用 <strong>128 位元</strong>位址，能提供近乎無限的位址數量。例如：<code>2001:0db8:85a3:0000:0000:8a2e:0370:7334</code>。 它是為了解決 IPv4 不足而設計的下一代協定。", color:colorGroups.group3, layer: '網路層'},
  {base:"NAT (Network Address Translation) - 網路位址轉譯", detail:"這是您家「路由器」的關鍵功能，像是一個「社區總機」。 您的ISP只會給您家一個「公共IP」，但您家裡卻有多台設備使用「私人IP」（例如 192.168.x.x）。 NAT技術能讓您<strong>多個內部設備透過共享這一個公共 IP 位址來上網</strong>，大大減緩了 IPv4 位址耗盡的問題。", color:colorGroups.group3, layer: '網路層'},
  {base:"ICMP (Internet Control Message Protocol) - <br>網際網路控制訊息協定", detail:"這不是用來傳送資料的，而是網路設備之間用來「<strong>回報錯誤和狀態</strong>」的簡訊系統。 最有名的應用就是 <strong>ping 指令</strong>，ping google.com 就是您的電腦發送一個ICMP「你在嗎？」的請求，Google的伺服器會回傳一個ICMP「我在！」的回應，用來<strong>診斷網路連線狀態</strong>。", color:colorGroups.group3, layer: '網路層'},
  ...
  {base:"MAC (Media Access Control) - 媒體存取控制", detail:"這是您網路卡（無論是有線還是Wi-Fi）的「<strong>身分證字號</strong>」。 這是一個<strong>全球唯一、燒錄在硬體中</strong>的實體位址（例如00:1A:2B:3C:4D:5E）。 在同一個區域網路內，設備之間就是靠這個位址來互相辨識和定位的。", color:colorGroups.group2, layer: '資料鏈結層'},
  ...
  {base:"ARP (Address Resolution Protocol) - 位址解析協定", detail:"這是在社區內「問路」的機制。您的電腦知道您印表機的IP位址（第三層，像是門牌號碼），但不知道它的MAC位址（第二層，身分證字號）。 於是您的電腦會透過ARP在網路中廣播：「嘿！請問<strong>IP是 192.168.1.100 的人，您的MAC位址是什麼？</strong>」。那台印表機收到後就會回覆：「是我是我！我的MAC是 xx:xx:xx:xx:xx:xx」。", color:colorGroups.group2, layer: '資料鏈結層'},
  {base:"CSMA/CD (Carrier Sense Multiple Access with <br>Collision Detection) - 載波感測多重存取/碰撞偵測", detail:"這是一套用在早期Hub網路上的「會議發言規則」。 它的運作方式為「<strong>先聽再說</strong>」：<br>1. <strong>Carrier Sense (先聽)</strong>：發言前先聽聽看有沒有人在講話。<br>2. <strong>Multiple Access (再說)</strong>：如果沒人講，我就開始講。<br>3. <strong>Collision <br>Detection (邊講邊聽)</strong>：我邊講邊聽，如果發現有人跟我同時講（發生碰撞），我們倆就立刻閉嘴，各自隨機等一小段時間，然後再重複第一步。", color:colorGroups.group2, layer: '資料鏈結層'},
  ...
  {base:"UTP (Unshielded Twisted Pair) - 無遮蔽雙絞線", detail:"這就是您最常見到的<strong>網路線</strong>（例如電腦連接到牆上插座的線）。它被稱為「無遮蔽」是因為線材外面沒有額外的金屬層；「雙絞」則是指裡面的八條銅線兩兩一對地纏繞在一起。這樣做的目的是利用電磁原理，<strong>抵銷</strong>來自外部（如電源線、日光燈）的雜訊干擾，以確保訊號穩定。", color:colorGroups.group1, layer: '實體層'},
  ...
  {base:"ADSL (Asymmetric Digital Subscriber Line) - <br>非對稱數位用戶線路", detail:"這就是早期的「電話線上網」技術，它利用傳統的電話銅線來提供寬頻上網。其最大的特點是「非對稱」，也就是<strong>下載速度遠高於上傳速度</strong>，這很符合當時一般人上網瀏覽（大量下載）的需求。", color:colorGroups.group1, layer: '實體層'},
];
