<template>
    <section class="w-full min-h-screen bg-bgg relative text-wordcolor overflow-hidden">
      <!-- 動畫圓環容器 -->
      <div
...
export default {
  ...
  setup(props, { emit }) {   // ← 拿到 emit
    ...
    function CIAanimation() {
      ...
      observer = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && animationContainer.value) {
    ...
    onUnmounted(() => {
      // 清理 observer，防止內存洩漏
      if (observer && animationContainer.value) {
        observer.unobserve(animationContainer.value)
        observer.disconnect()
      }
    })
